...
import tools
from utils import MLPBase, MLPBaseDist
import torchfold
...
class ActorVanilla(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action, min_std = 0.1, init_std = 1):
        super(ActorVanilla, self).__init__()
        self.max_action = max_action
        self.base = MLPBaseDist(state_dim, action_dim, min_std, init_std)

    def forward(self, x, decoder=None):
        x = decoder(x).mode()
        return self.base(x)

    def change_morphology(self, x):
        pass
...
class ActorGraphPolicy(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_children,batch_size = 1, msg_dim = 32, 
                    max_action = 1, disable_fold = False, td = True, bu = True):
    ...
    def forward(self, state, decoder = None):
        state = decoder(state).mode()
        ...
        if not self.disable_fold:
            ...
            self.a = []
        state = state[:, :self.state_dim * self.num_limbs]
        ...
        x = torch.squeeze(self.action)
        if len(x.shape) == 1:
            x = x.unsqueeze(0)
        if tmp_x:
            x = x.view(tmp_x, tmp_y, x.shape[-1])
        dist = tools.SafeTruncatedNormal(x, 0.05, -1, 1)
